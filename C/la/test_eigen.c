...
void r8bin_print ( int bin_num, int bin[], double bin_limit[], char *title )
...
*/
{
  ...
  printf ( "%s\n", title );
  printf ( "\n" );
  printf ( "   Index     Lower Limit   Count     Upper Limit\n" );
  printf ( "\n" );
  printf ( "  %6d                  %6d  %14g\n", 0, bin[0], bin_limit[0] );
  for ( i = 1; i <= bin_num; i++ )
  {
    printf ( "  %6d  %14g  %6d  %14g\n", i, bin_limit[i-1], bin[i], bin_limit[i] );
  }
...
void r8nsymm_gen ( int n, double lambda_mean, double lambda_dev, int *seed, 
  double a[], double ql[], double qr[], double lambda[] )
...
/*
  ...
  Parameters:
    ...
    Output, double QL[N*N], the left eigenvector matrix.

    Output, double QR[N*N], the right eigenvector matrix.
...
*/
  r8vec_normal_ab ( n, lambda_mean, lambda_dev, seed, lambda );
/*
  Get random orthogonal matrices QL and QR.
*/
  r8mat_orth_uniform ( n, seed, ql );
  r8mat_orth_uniform ( n, seed, qr );
/*
...
*/
  for ( i = 0; i < n; i++ )
  {
    for ( j = 0; j < n; j++ )
    {
      ...
      for ( k = 0; k < n; k++ )
      {
        a[i+j*n] = a[i+j*n] + ql[i+k*n] * lambda[k] * qr[j+k*n];
      }
    }
  }
  return;
...
void r8symm_gen ( int n, double lambda_mean, double lambda_dev, int *seed, 
